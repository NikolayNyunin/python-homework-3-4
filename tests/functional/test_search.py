...
class TestSearch:
    @pytest.mark.asyncio
    async def test_search_zero(self, client: AsyncClient):
        response = await client.get('/links/search?original_url=https://404.com/')
    ...
    @pytest.mark.asyncio
    async def test_search_one(self, client: AsyncClient, test_link1: Link):
        response = await client.get(f'/links/search?original_url={test_link1.original_url}')
    ...
    @pytest.mark.asyncio
    async def test_search_two(self, client: AsyncClient, test_link1: Link, test_link2: Link):
        response = await client.get(f'/links/search?original_url={test_link1.original_url}')
